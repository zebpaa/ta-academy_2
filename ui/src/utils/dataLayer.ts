...
export class DataLayer {
  ...
  private async findInDataLayer(
    ...
    const checkIsQueryPassed = (e: DataLayerEvent): boolean =>
      deepMatch(e, eventQuery);
  ...
  public async waitForDataLayer(
    ...
    return await waitFor(() => this.findInDataLayer(event), options).catch(
      (e) => this.throwNoEventFoundError(e, event)
    );
  }
  ...
  private throwNoEventFoundError = (
    ...
    const errorLog = Object.entries(event)
      .map(
        ([eventName, eventValue]) => `${eventName}: "${eventValue.toString()}"`
      )
      .join('\n');
    throw new Error(
